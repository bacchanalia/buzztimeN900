...
def main():
    while True:
        tgt = list (time.localtime ())
        tgt[3] = (tgt[3] if tgt[4] < 55 else tgt[3] + 1) % 24   # hours
        tgt[4] = (tgt[4] - tgt[4] % 5 + 5) % 60                 # minutes
        tgt[5] = 0                                              # seconds
        tgt = time.struct_time(now)
        time.sleep (time.mktime (tgt) - time.time ())
        ...
        mapM_ (doVibeList) (vibes (tgt.tm_hour) (tgt.tm_min))
